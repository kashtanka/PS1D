      ...
      do iz=2,nz-1                        ! vertical advection
         vat(iz)= 0.5*(w(iz)+w(iz+1))*(th(iz+1,1)-th(iz,1))/
     :            (0.5*(dz(iz+1)+dz(iz)))
...
!     :   (0.5*(dz(iz)+dz(iz+1)))
 ...
 !    :       .and.0.5*(z(iz)+z(iz-1)).le.zi_rec) then
     ...
     :      .ge.hbl.and.0.5*(z(iz-1)+z(iz)).lt.hbl) then
            ...
            gambl=(th(iz-1,2)-th(iz-2,2))/dz(iz-1)
            gamfa=(th(iz+2,2)-th(iz+1,2))/dz(iz+2)
...
!     :            (zi_rec - z(iz-1))+
     :             w_e*(delta_th)/dz(iz-1)
            ...
            gambl = (qv(iz-1,2)-qv(iz-2,2))/dz(iz-1)
            gamfa = (qv(iz+2,2)-qv(iz+1,2))/dz(iz+2)
